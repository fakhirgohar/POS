    ...
    public partial class frmSupplierPayment : Form
    {
        ...
        private void cboSupp_SelectedValueChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(cboSupp.Text.Trim()))
            {
                ...
                DataTable dtPurchase = SqlQuery.Read(con, tran, Global.ConnectionString, "SELECT BillNo FROM Inventory_Purchase WITH(NOLOCK)");
        ...
        private void LoadData(string BillNo)
        {
            ...
            dtDetail.Rows.Clear();
            dtDetail = SqlQuery.Read(con, tran, Global.ConnectionString, $"SELECT Sno, BillNo, ItemCode, SerialNo, StoreID, PPrice, Qty FROM Inventory_PurchaseDetail WHERE BillNo = '{BillNo}'");
            dgvDetail.DataSource = dtDetail;
            ResetDgvColumns();
        }
